...
const UserProfileMenu = ({ userData, navigate, onLogout }) => {
  ...
  const [userName, setUserName] = useState('');
  const [userInitials, setUserInitials] = useState('U');
  const [isLoading, setIsLoading] = useState(false); // Start with false since we have instant access

  const { logout: authLogout, planInfo: contextPlanInfo, user: contextUser } = useAuth(); // Get plan from RAM and user from context
  ...
  // Main function to update user info - OPTIMIZED for instant access
  const updateUserInfo = useCallback(async () => {
    // Don't set loading to true - we have instant access from context
    console.log('🔍 UserProfileMenu: Starting updateUserInfo (optimized)...');
    
    let userInfo = null;
    let planInfo = null;

    // PRIORITY 1: Get user from context (RAM) - INSTANT
    ...
    // PRIORITY 1: Get plan from context (RAM) - INSTANT ACCESS, NO API CALL NEEDED
    if (contextPlanInfo && contextPlanInfo.plan) {
      ...
      // PRIORITY 2: Get plan from localStorage (fast)
      planInfo = getFromStorage('userInfo');
      if (planInfo && planInfo.plan) {
        console.log('✅ Found plan data in localStorage["userInfo"]:', planInfo.plan);
      } else {
        // PRIORITY 3: Check other localStorage keys
      ...
      // PRIORITY 4: Only fetch from API if not found anywhere (last resort, non-blocking)
      if (!planInfo || !planInfo.plan) {
        console.log('🌐 No plan found, fetching from API in background...');
        // Don't await - fetch in background so UI doesn't block
        fetchPlanFromAPI().then(apiPlan => {
          if (apiPlan) {
            console.log('✅ Got plan from API (background):', apiPlan);
            // Update state when API returns
            setUserPlan(apiPlan);
          }
        }).catch(err => {
  ...
  // Watch for changes in context plan info (instant updates when plan changes in context)
  useEffect(() => {
    if (contextPlanInfo && contextPlanInfo.plan) {
  ...
  // Removed loading state - we have instant access from context
